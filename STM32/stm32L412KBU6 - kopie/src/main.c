  ...
  while (1)
{
    ...
    char I2C_Buffer[25] = {0};
    char UART_Buffer[50] = {0};

    // I2C Transmit
    int ret = HAL_I2C_Master_Transmit(&hi2c1, SHT40_I2C_ADDR, I2C_Buffer, 2, 5000);
    HAL_Delay(500); 
    printf("I2C Transmission !\r\n");
    if (ret != HAL_OK)
    {
    ...
    {
        // I2C Receive
        ret = HAL_I2C_Master_Receive(&hi2c1, SHT40_I2C_ADDR, I2C_Buffer, 16, 5000);
        if (ret == HAL_OK)
        {
            // Process I2C_Buffer for sensor data (e.g., temperature or humidity)
            int lux = 123; // Replace with actual sensor data processing
            sprintf(UART_Buffer, "Lux: %d\r\n", lux);
        }
        else
        {
            printf("I2C Reception Failed!\r\n");
        }
    }

    // UART Transmit
    HAL_UART_Transmit(&huart2, (uint8_t *)UART_Buffer, strlen(UART_Buffer), 5000);
